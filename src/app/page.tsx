...
export default function Home() {
  ...
  return (
   <div className=" flex justify-center  " >
    ...
    <div className=" p-7 bg-[#1B1D1F] rounded-lg  my-8 mx-4 md:mt-24  text-center relative z-10 w-[1024px]  " >
       <picture className=" absolute top-0 left-0 w-full h-[200px] -z-10 " >
        <Image alt="" src={"/vector.svg"} fill />
       </picture>
       <h1 className=" text-[2rem] mb-4" >Our Collection</h1>
       ...
       <div className=" space-x-4 mt-9 " >
        <button className={`rounded-lg p-2 ${ currentFilter === 'all' ? 'bg-[#6F757C]':'hover:bg-[#6F757C]' }  `}  onClick={handleAllProduct} >All Products</button>
        <button className={`rounded-lg p-2 ${ currentFilter === 'available' ? 'bg-[#6F757C]':'hover:bg-[#6F757C]' }  `}  onClick={handleAvailableProduct}  >Available Now</button>
       </div>
